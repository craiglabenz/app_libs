...
void main() {
  ...
  group('LocalMemorySource.setItems should', () {
    ...
    test('set items with pagination then without [mock mem]', () async {
      ...
      await mockMem.setItems([item2, item3], details);
      verify(() => cache.setCacheKey(details.cacheKey, {item2.id!, item3.id!}))
          .called(1);
      verifyNever(
        () => cache.setPaginatedCacheKey(
          noPaginationCacheKey: any(named: 'noPaginationCacheKey'),
    ...
    test('set items with all pagination requests different page [real mem]',
        () async {
      await mem.setItems([item, item2], paginationDetails);
      await fullyContains(mem, [item, item2], requests: [paginationDetails]);
    ...
    test('set items with all pagination requests different page [mock mem]',
        () async {
      await mockMem.setItems([item, item2], paginationDetails);
      verifyNever(() => cache.setCacheKey(paginationDetails.cacheKey, any()));
      verify(
    ...
    test('set items with set name [real mem]', () {
      mem.setItems([item, item2], details);
      fullyContains(mem, [item, item2], requests: [details]);

      mem.setItems([item2, item3], abcDetails);
      fullyContains(mem, [item], requests: [details]);
      fullyContains(mem, [item2], requests: [details, abcDetails]);
      fullyContains(mem, [item3], requests: [abcDetails]);
    });

    test('set items with set name [mock mem]', () {
      mockMem.setItems([item, item2], details);
      verify(() => cache.setCacheKey(details.cacheKey, {item.id!, item2.id!}))
          .called(1);
      verifyNever(
        () => cache.setPaginatedCacheKey(
          noPaginationCacheKey: any(named: 'noPaginationCacheKey'),
      ...
      mockMem.setItems([item2, item3], abcDetails);
      verify(
...
Future<void> fullyContains(
  ...
  for (final item in items) {
    ...
    expect(
        (await mem.getById(item.id!, details)).getOrRaise().item, equals(item));
