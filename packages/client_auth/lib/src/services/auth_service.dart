...
/// Introduces email/password authentication to an [AuthService].
mixin EmailAuthService on AuthService {
  /// {@template signUp}
  ...
  /// Completes the sign up process started with [signUp] by submitting the
  /// email verification token.
  Future<AuthSuccess> verifyEmail(String token);

  /// {@template logInWithEmailAndPassword}
  /// Restores the session of an existing user with the provided [email] and
  /// [password].
  /// {@endtemplate}
  Future<AuthSuccess> logInWithEmailAndPassword({
    required String email,
    required String password,
  });
}

/// Introduces social authentication to an [AuthService].
mixin SocialAuthService on AuthService {
  /// {@template logInWithApple}
  /// Starts the Sign In with Apple Flow.
  /// {@endtemplate}
  Future<AuthSuccess> logInWithApple();

  /// {@template logInWithGoogle}
  /// Starts the Sign In with Google Flow.
  /// {@endtemplate}
  Future<AuthSuccess> logInWithGoogle();
}
