...
final details = RequestDetails<_TestModel>.read();
...
void main() {
  ...
  group('FirestoreSource.getByIds should', () {
    ...
    test('return empty sets', () async {
      ...
      expect(
        ...
        <String>{homerSimpson.id!, bartSimpson.id!},
      );
    });
    ...
    test('applyFilters', () async {
      ...
      final filterDetails = RequestDetails<_TestModel>.read(filters: [filter]);
  ...
  group('FirestoreSource.setItem should', () {
    ...
    test('save a known item', () async {
      when(() => docRefMock.set(bartSimpsonCreationData))
          .thenAnswer((_) async {});
    ...
    test('save a new item', () async {
      when(() => colRefMock.add(bartSimpsonCreationData)).thenAnswer(
        (_) async => docRefMock,
      );
      when(docRefMock.get).thenAnswer((_) async => docSnapshotMock);
...
class _FirestoreFilterMock extends ReadFilter<_TestModel> with FirestoreFilter {
  const _FirestoreFilterMock(this._filterQuery);

  final Query<Json> Function(Query<Json>) _filterQuery;

  @override
  bool predicate(_TestModel obj) => throw UnimplementedError();
